...
def prep_db():
	print(f"host=localhost port={DB_PORT} dbname=postgres user={DB_USER} password={DB_PASSWORD}")
	with psycopg.connect(f"host=localhost port={DB_PORT} dbname=postgres user={DB_USER} password={DB_PASSWORD}", autocommit=True) as conn:
		res = conn.execute(f"SELECT 1 FROM pg_database WHERE datname='{DB_NAME}'")
		if len(res.fetchall()) == 0:
			conn.execute("create database test;")
		with psycopg.connect(f"host=localhost port={DB_PORT} dbname={DB_NAME} user={DB_USER} password={DB_PASSWORD}") as conn:
